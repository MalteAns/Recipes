...
import recipes.composeapp.generated.resources.select_image
...
actual fun rememberImagePickerLauncher(onResult: (PickedImageData) -> Unit): ImagePickerLauncher {
    ...
    val dialogTitle = stringResource(Res.string.select_image)
    return remember {
        ImagePickerLauncher(
            onLaunch = {
                scope.launch {
        ...
        if(file != null) {
            try {
                ...
                PickedImageData(
                    filename = file.name,
                    bytes = bytes,
                    mimeType = mimeType ?: return@withContext null
                )
            } catch(_: Exception) {
